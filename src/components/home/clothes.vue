<template>
     <div class="clothesWrapper">
        <ul class="clothesBox">
            <router-link v-for="(item, index) in clothesList" 
                        :key="index" 
                        class="clothesBody" 
...
export default {
    ...
    methods:{
        setStore (index) {
            ...
            axios.get('api/updateViews?goods_id='+ this.clothesList[index].goods_id +'&view='+ this.clothesList[index].views)
            this.$store.commit('setVal',this.clothesList[index]);
        },
        getGoods () {
            axios.get('api/getClothesGoodsByPage?pageSize='+ this.pageSize +'&currentPage='+ (this.currentPage-1)).then(res => {
            // console.log(res.data)
            this.clothesList = res.data
            });
            axios.get('api/getClothesGoodsCount').then(res => {
                // console.log(res)
